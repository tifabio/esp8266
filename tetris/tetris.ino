...
//  Serial.begin(9600);
  ...
  lcd.setCursor(3,0); // Place cursor row 4, 1st line (counting from 0)
  lcd.print("TETRIS");
  lcd.setCursor(5,1); // Place cursor row 6, 2nd line (counting from 0)
  lcd.print("ESP8266");
  delay(800);
...
int get_key()
{
   ...
   valX = map(valX, 0, 1023, 1, 2);
   //delay(100);
   int valY = analogRead(PIN_Y);
   valY = map(valY, 0, 1023, 3, 4);
...
void loop() {
  ++counter;
  key = get_key(); // read the value from the sensor & convert into key press
